...
<!--	import { controlled, gameId, players, resetGame } from '$lib/gameState.js';-->
...
<!--		const getSlots = () => getAvailableLobbySlots($gameId).then(res => filled = res);-->
<!--		getSlots();-->
<!--		const interval = setInterval(getSlots, 2000);-->
...
<!--			{#if $controlled === slot}-->
<!--				(You)-->
<!--			{:else if filled?.findIndex(x => x === slot) === -1 }-->
<!--				(Filled)-->
<!--			{/if}-->
<!--		</div>-->
<!--	{/each}-->
